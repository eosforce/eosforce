...
void apply_context::execute_inline( action&& a ) {
   ...
   if( control.head_block_num() > 4470000 ) {
      EOS_ASSERT(a.name != N(onfee), action_validate_exception, "no call");
   }
...
void apply_context::schedule_deferred_transaction( const uint128_t& sender_id, account_name payer, transaction&& trx,
   ...
   if( auto ptr = db.find<generated_transaction_object, by_sender_id>(boost::make_tuple(receiver, sender_id))) {
      ...
      db.modify<generated_transaction_object>(*ptr, [&]( auto& gtx ) {
         ...
         gtx.expiration = gtx.delay_until +
...
int
apply_context::db_store_i64( uint64_t scope, uint64_t table, const account_name& payer, uint64_t id, const char *buffer,
                             size_t buffer_size ) {
   return db_store_i64(receiver, scope, table, payer, id, buffer, buffer_size);
}

int apply_context::db_store_i64( uint64_t code, uint64_t scope, uint64_t table, const account_name& payer, uint64_t id,
